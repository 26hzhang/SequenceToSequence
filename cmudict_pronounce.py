...
def create_configurations():
    # dataset parameters
    tf.flags.DEFINE_string("dataset_name", "cmudict", "dataset name")
    tf.flags.DEFINE_string("raw_data_dir", "dataset/raw/cmudict", "path to the raw data directory")
    tf.flags.DEFINE_string("save_dir", "dataset/data/cmudict", "path to the processed dataset directory")
    tf.flags.DEFINE_string("target_vocabulary", "dataset/data/cmudict/metadata.json", "path to the target vocabulary")
    tf.flags.DEFINE_string("dataset", "dataset/data/cmudict/dataset.json", "path to the train and test datasets")
    tf.flags.DEFINE_integer("max_size", 20, "maximal size of chars in a word or phoneme in a pronounce")
    tf.flags.DEFINE_integer("min_size", 5, "minimal size of chars in a word or phoneme in a pronounce")
    ...
    tf.flags.DEFINE_boolean("only_top_attention", False, "apply attention mechanism only on the top decoder layer")
    tf.flags.DEFINE_integer("num_units", 256, "number of hidden units in each layer")
    tf.flags.DEFINE_integer("num_layers", 3, "number of layers for encoder and decoder")
    tf.flags.DEFINE_integer("emb_dim", 128, "embedding dimension for encoder and decoder input words/tokens")
    ...
    tf.flags.DEFINE_string("checkpoint_path", "ckpt/cmudict/", "path to save model checkpoints")
    tf.flags.DEFINE_string("summary_path", "ckpt/cmudict/summary/", "path to save summaries")
...
def ids_to_sentence(predict_ids, rev_target_dict, target_dict):
    ...
    :return: predicted list of words
    """
    ...
    for predict in predict_ids:
        ...
        for i in range(shapes[-1]):
            word = [rev_target_dict.get(idx, UNK) for idx in predict[:, i] if idx not in special_tokens]
